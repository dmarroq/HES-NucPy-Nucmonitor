...
def merge_gridfs_files_to_json():
    """
    ...
    db = client['data']
    fs = GridFS(db, collection='clean_nuc')
...
class Nucmonitor(Resource):
    ...
    def get_rte_data(self, start_date, end_date):
        rte_url = "http://127.0.0.1:5000/nucpy/v1/rte"  # RTE endpoint URL
    ...
    def get_mongo_data(self):
        mongo_url = "http://127.0.0.1:5000/nucpy/v1/raw"  # Mongo endpoint URL
        mongo_response = requests.get(mongo_url)
